...
def BuildReachableFileSet(entry_classes, reachability_tree, header_mapping,
  ...
  transpiled_entry_files.extend(
      transpiled_file for transpiled_file in reachability_tree
  ...
  if input_archive in archive_source_file_mapping:
    ...
    if unreachable_object_names := [
      ...
      if len(unreachable_object_names) == len(source_files):
        j2objc_cmd = f'cp {pipes.quote(dummy_archive)} {pipes.quote(output_archive)}'
      else:
        cmd_env['ZERO_AR_DATE'] = '1'
        # Copy the input archive to the output location
        j2objc_cmd += f'cp {pipes.quote(input_archive)} {pipes.quote(output_archive)} && '
        # Make the output archive editable
        j2objc_cmd += f'chmod +w {pipes.quote(output_archive)} && '
        ...
        j2objc_cmd += f"{pipes.quote(xcrunwrapper)} ar -d -s {pipes.quote(output_archive)} {' '.join((pipes.quote(uon) for uon in unreachable_object_names))} && "
        # Update the table of content of the archive file
        j2objc_cmd += (
            f'{pipes.quote(xcrunwrapper)} ranlib {pipes.quote(output_archive)}'
        )
    else:
      j2objc_cmd = f'cp {pipes.quote(input_archive)} {pipes.quote(output_archive)}'
  else:
