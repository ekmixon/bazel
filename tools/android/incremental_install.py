...
class Adb(object):
  ...
  def GetInstallTime(self, package):
    ...
    if match := re.search("firstInstallTime=(.*)$", six.ensure_str(stdout),
                          re.MULTILINE):
      return match[1]
    else:
      return None
...
def UploadDexes(adb, execroot, app_dir, temp_dir, dexmanifest, full_install):
  ...
  files_to_push.extend((new_manifest[dex].input_file,
                        targetpath.join(dex_dir, dex))
...
def UploadNativeLibs(adb, native_lib_args, app_dir, full_install):
  ...
  device_manifest = (None if full_install else adb.Pull(
  ...
  install_manifest = [
      f"{six.ensure_str(name)} {checksum}"
